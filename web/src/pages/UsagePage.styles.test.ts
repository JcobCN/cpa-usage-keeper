...
describe('UsagePage toolbar styles', () => {
  ...
  it('keeps the Analysis chart presentation aligned with the redesigned Analysis dashboard', () => {
    ...
    expect(analysisPanelStyles).toMatch(/\.heatmapLegendRamp\s*\{[\s\S]*?linear-gradient\(90deg, #fff5eb, #ffd8a8, #ffa94d, #e8590c, #5c2b0e\)/)
    expect(analysisPanelStyles).toMatch(/\.heatmapCardDark \.heatmapLegendRamp\s*\{[\s\S]*?linear-gradient\(90deg, #1a1108, #4a2a12, #a0400a, #ff7b00, #ffe066\)/)
